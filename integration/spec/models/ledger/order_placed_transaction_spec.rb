...
RSpec.describe Ledger::OrderPlacedTransaction, type: :model do
  subject(:transaction) { build(:order_placed_transaction) }
  ...
  it 'applies no net change to the balance of the delivery fees revenue account' do
    expect(transaction).not_to change_balance_of(:delivery_fees)
  end

  it 'retains the platform fee in the platform fees revenue account' do
    expect(transaction).to change_balance_of(:platform_fees).by(order.platform_fee, order.currency)
  end
end
